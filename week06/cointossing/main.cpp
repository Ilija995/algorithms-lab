...
void testcases() {
    ...
    for (int i = 0; i < m; ++i)
    {
        std::cin >> a >> b >> c;
        if (c == 1) --scores[a];
        else if (c == 2) --scores[b];
        else edges.push_back(std::make_pair(a, b));
    }

    int min_score = INT_MAX, score, sum_score = 0;
    for (int i = 0; i < n; ++i)
    {
        std::cin >> score;
        scores[i] += score;
    ...
    if (min_score < 0 || sum_score != edges.size())
    {
        std::cout << "no\n";
        return;
    }
    ...
    {
        eaG.addEdge(i + 1, 1 + edges.size() + edges[i].first, 1);
        eaG.addEdge(i + 1, 1 + edges.size() + edges[i].second, 1);
    }
    for (int i = 0; i < n; ++i)
        eaG.addEdge(1 + edges.size() + i, 1 + edges.size() + n, scores[i]);
	...
	long flow = boost::push_relabel_max_flow(G, 0, 1 + edges.size() + n);
	std::cout << ((flow == edges.size()) ? "yes" : "no") << std::endl;
}
